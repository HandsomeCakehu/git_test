...
/************
void PclTestCore::remove_ground(const pcl::PointCloud<pcl::PointXYZI>::Ptr in,const pcl::PointCloud<pcl::PointXYZI>::Ptr out){
    //创建分割时所需要的模型系数对象，coefficients及存储内点的点索引集合对象inliers
        ...
        {
            std::cout<<"error! Could not found any inliers!"<<endl;
        }
        // extract ground
        // 从点云中抽取分割的处在平面上的点集
        pcl::ExtractIndices<pcl::PointXYZI> extractor;//点提取对象
        extractor.setInputCloud(in);
        extractor.setIndices(inliers);

        //这一步很重要！！！true是滤除平面点，false是保留地面点
        extractor.setNegative(true);
        //将保留的地面点存放在指针cloud_filtered里面
        extractor.filter(*out);
}
**********************************/
...
/////////////////////////////////////////////////////////////////////
//滤除界值下方的点云
void PclTestCore::clip_below(double clip_below_height, const pcl::PointCloud<pcl::PointXYZI>::Ptr in,
                             const pcl::PointCloud<pcl::PointXYZI>::Ptr out)
{
    pcl::ExtractIndices<pcl::PointXYZI> cliper;
...
#pragma omp for
    for (size_t i = 0; i < in->points.size(); i++)
    {
        if (in->points[i].z < clip_below_height)
        {
            indices.indices.push_back(i);
        }
    }
...
void PclTestCore::voxelgrid( const pcl::PointCloud<pcl::PointXYZI>::Ptr in,
                             const pcl::PointCloud<pcl::PointXYZI>::Ptr out)
{
...
void PclTestCore::remove_cloud_Seg(const pcl::PointCloud<pcl::PointXYZI>::Ptr in,
                      const pcl::PointCloud<pcl::PointXYZI>::Ptr out)
{
...
void PclTestCore::remove_cloud_Rad(const pcl::PointCloud<pcl::PointXYZI>::Ptr in,
                      const pcl::PointCloud<pcl::PointXYZI>::Ptr out)
{
    pcl::RadiusOutlierRemoval<pcl::PointXYZI> pcFilter;  //创建滤波器对象
    pcFilter.setInputCloud(in);             //设置待滤波的点云
    pcFilter.setRadiusSearch(0.5);               // 设置搜索半径
    pcFilter.setMinNeighborsInRadius(20);      // 设置一个内点最少的邻居数目
    pcFilter.filter(*out);        //滤波结果存储到cloud_filtered
}
...
void PclTestCore::point_cb(const sensor_msgs::PointCloud2ConstPtr &in_cloud_ptr)
{
    ...
    pcl::PointCloud<pcl::PointXYZI>::Ptr cliped_below_ptr(new pcl::PointCloud<pcl::PointXYZI>);
    clip_below(CLIP_BELOW_HEIGHT, cliped_pc_ptr, cliped_below_ptr);

    pcl::PointCloud<pcl::PointXYZI>::Ptr remove_close(new pcl::PointCloud<pcl::PointXYZI>);

    remove_close_pt(MIN_DISTANCE, cliped_below_ptr, remove_close);

    pcl::PointCloud<pcl::PointXYZI>::Ptr remove_voxel(new pcl::PointCloud<pcl::PointXYZI>);
    voxelgrid(remove_close,remove_voxel);

    pcl::PointCloud<pcl::PointXYZI>::Ptr remove_Seg(new pcl::PointCloud<pcl::PointXYZI>);
    remove_cloud_Seg(remove_voxel,remove_Seg);

    pcl::PointCloud<pcl::PointXYZI>::Ptr remove_Rad(new pcl::PointCloud<pcl::PointXYZI>);
    remove_cloud_Rad(remove_Seg,remove_Rad);
    ...
    XYZI_to_RTZColor(remove_Rad, organized_points,
    ...
    extract_ground.setInputCloud(remove_Rad);
    ...
    viewer.showCloud(no_ground_cloud_ptr);
